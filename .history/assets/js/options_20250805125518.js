...
function applyUserSettings() {
  ...
  if (header) {
    header.classList.remove('fixed-always', 'floating');
    if (settings.fixedHeader === 'always') {
      header.classList.add('fixed-always');
    } else if (settings.fixedHeader === true) {
      header.classList.add('floating');
    }
  }

  // --- Кольори: Акцент, Фон, Текст ---
  if (settings.accentColor) {
