...
function fadeInMediaSidebar() {
  const sidebar = document.querySelector(".media-sidebar-fadein");
  if (sidebar) {
    sidebar.classList.add("start"); // задаємо стартовий стан

    requestAnimationFrame(() => {
      setTimeout(() => {
        sidebar.classList.remove("start"); // запускаємо перехід вниз
      }, 30); // маленька затримка — щоби браузер встиг застосувати початковий стан
    });
  }
}
