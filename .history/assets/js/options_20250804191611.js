...
function updateActiveButtons() {
  ...
  const accentColor = settings.accentColor || '#ff0000'; // стандартний акцент
...
svgInsertions.forEach(({ containerId, svgPath }) => {
  ...
  // Створюємо тег <img> з SVG
  const img = document.createElement('img');
  img.src = svgPath;
  img.alt = 'SVG Image';
  img.classList.add('svg-inserted');
...
document.querySelector('.button-open-options-button').onclick = function () {
  ...
  document.querySelector('.button-hide-button-options-icon').onclick = function () {
    document.querySelector('.button-open-options-container-content').classList.toggle('open');
  };
