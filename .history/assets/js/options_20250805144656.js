function updateUserSetting(key, value) {
  ...
  applyUserSettings(settings); // Передаємо явно
...
// Застосування всіх збережених налаштувань
...
document.addEventListener('DOMContentLoaded', () => {
  applyUserSettings(); // Автоматичне застосування
});

document.querySelectorAll('[data-color-setting]').forEach(input => {
  input.addEventListener('input', () => {
    const settingKey = input.dataset.colorSetting; // напр. 'themeColor'
    const colorValue = input.value;
    updateUserSetting(settingKey, colorValue);
  });
});
