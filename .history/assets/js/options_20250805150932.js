...
const COLOR_KEY = "customColorSettings";
...
function applyColors() {
  ...
  else root.style.setProperty('--accent-color', '#C778DD');

  if (colors.background) root.style.setProperty('--background-color', colors.background);
  else root.style.setProperty('--background-color', '#ffffff');

  if (colors.text) root.style.setProperty('--text-color', colors.text);
  else root.style.setProperty('--text-color', '#000000');

  // Підставимо значення у поля
  document.getElementById('accentColorPicker').value = colors.accent || '#C778DD';
  document.getElementById('backgroundColorPicker').value = colors.background || '#ffffff';
  document.getElementById('textColorPicker').value = colors.text || '#000000';
}
