...
function initColorPickers() {
  ...
  if (accent) {
    accent.value = settings.accentColor || '#C778DD';
    accent.addEventListener('input', () => {
      updateUserSetting('accentColor', accent.value);
    });
  }

  if (background) {
    background.value = settings.backgroundColor || '#0F0F0F';
    background.addEventListener('input', () => {
      updateUserSetting('backgroundColor', background.value);
    });
  }

  if (text) {
    text.value = settings.textColor || '#FFFFFF';
    text.addEventListener('input', () => {
      updateUserSetting('textColor', text.value);
    });
  }
