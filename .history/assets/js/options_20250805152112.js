...
// === Ключ окремих кольорів ===
const COLOR_KEY = 'colorSettingsCustom';
...
// === Застосувати кольори на сайт ===
function applyCustomColors() {
  const { accent, background, text } = getCustomColors();
  ...
  if (backgroundInput && background) backgroundInput.value = background;
  if (textInput && text) textInput.value = text;
}

// === Змінити певний колір ===
...
// === Скинути певний колір ===
function resetCustomColor(type) {
  const colors = getCustomColors();
  delete colors[type];
  localStorage.setItem(COLOR_KEY, JSON.stringify(colors));
  applyCustomColors();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Слухачі зміни кольорів
  const accentInput = document.getElementById('accentColorPicker');
  const backgroundInput = document.getElementById('backgroundColorPicker');
  const textInput = document.getElementById('textColorPicker');

  if (accentInput) {
    accentInput.addEventListener('input', () => updateCustomColor('accent', accentInput.value));
  }
  if (backgroundInput) {
    backgroundInput.addEventListener('input', () => updateCustomColor('background', backgroundInput.value));
  }
  if (textInput) {
    textInput.addEventListener('input', () => updateCustomColor('text', textInput.value));
  }
  ...
  if (accentReset) {
    accentReset.addEventListener('click', () => resetCustomColor('accent'));
  }
  if (backgroundReset) {
    backgroundReset.addEventListener('click', () => resetCustomColor('background'));
  }
  if (textReset) {
    textReset.addEventListener('click', () => resetCustomColor('text'));
  }
});
