...
function updateActiveButtons() {
  ...
  // Всі кнопки в .button-open-options-list
  const buttons = document.querySelectorAll('.button-open-options-list button');

  buttons.forEach(button => {
    const onclick = button.getAttribute('onclick');
    if (!onclick) {
      // Якщо кнопка без onclick — просто скидаємо active
    ...
    // Активуємо кнопку, якщо збережене значення === value, або якщо і те, і інше undefined
