...
function loadColorSettings() {
  ...
  if (colorPicker) {
    colorPicker.value = accentColor;
    document.body.style.setProperty('--accent-color', accentColor); // Застосування кольору до стилів
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Обробник для кольорового пікера
  const colorPicker = document.getElementById('accentColorPicker');
  if (colorPicker) {
    colorPicker.addEventListener('input', () => {
      saveColorSetting('accentColor', colorPicker.value);
      document.body.style.setProperty('--accent-color', colorPicker.value); // Застосування кольору до стилів
    });
  }
