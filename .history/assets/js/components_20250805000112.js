...
async function loadComponent({ id, htmlPath, scriptPath }) {
  ...
  try {
    ...
    if (scriptPath) {
      ...
      // Додайте обробник помилок для скрипта
      script.onerror = () => {
        console.error(`[Script Loader Error]: Failed to load ${scriptPath}`);
      };
    }
...
async function loadAllComponents() {
  ...
  await Promise.all(loadPromises); // Завантажити всі компоненти паралельно
