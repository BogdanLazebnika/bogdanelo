...
function updateUserSetting(key, value) {
  ...
  // Оновлюємо класи активних кнопок
  updateActiveButtons();
}

// Застосувати налаштування з localStorage
function applyUserSettings() {
  const settings = JSON.parse(localStorage.getItem('siteSettings')) || {};
  const root = document.documentElement;

  // Тема (додаємо або прибираємо клас body)
...
// Кнопки і інпути підсвічуємо згідно з налаштуваннями
function updateActiveButtons() {
  const settings = JSON.parse(localStorage.getItem('siteSettings')) || {};
  const buttons = document.querySelectorAll('.button-open-options-list-list-two-level button');
  ...
  const backgroundColor = settings.backgroundColor || '#0F0F0F';
  ...
  if (backgroundColorPicker) backgroundColorPicker.value = backgroundColor;

  // Скидання кнопок (активність)
  ...
  if (accentResetBtn) {
    if (!settings.accentColor) accentResetBtn.classList.add('active');
    else accentResetBtn.classList.remove('active');
    accentResetBtn.onclick = () => updateUserSetting('accentColor', undefined);
  }

  if (textResetBtn) {
    if (!settings.textColor) textResetBtn.classList.add('active');
    else textResetBtn.classList.remove('active');
    textResetBtn.onclick = () => updateUserSetting('textColor', undefined);
  }

  if (backgroundResetBtn) {
    if (!settings.backgroundColor) backgroundResetBtn.classList.add('active');
    else backgroundResetBtn.classList.remove('active');
    backgroundResetBtn.onclick = () => updateUserSetting('backgroundColor', undefined);
  }
}
