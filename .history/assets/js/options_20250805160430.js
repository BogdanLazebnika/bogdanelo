...
(function() {
  ...
  // Завантажити налаштування з localStorage або повернути дефолт
  function loadColors() {
    try {
  ...
  // Зберегти налаштування у localStorage
  ...
  function init() {
    ...
    if (!accentInput || !textInput || !backgroundInput) {
      // Якщо пікери не знайдені — зупиняємось
      return;
    }

    // Встановити початкові значення в пікери
    ...
    // Обробники для пікерів кольорів
    ...
    // Обробники скидання кольорів (за умови, що є кнопки скидання з відповідними id)
