...
// Функція для збереження кольору в локальному сховищі
function saveColorSetting(key, value) {
  localStorage.setItem(key, value);
}

// Функція для завантаження кольору з локального сховища
function loadColorSetting(key, defaultValue) {
  return localStorage.getItem(key) || defaultValue;
}

// Ініціалізація кольорових налаштувань
document.addEventListener('DOMContentLoaded', () => {
  // Завантажуємо кольори з локального сховища
  const accentColor = loadColorSetting('accentColor', '#C778DD');
  const backgroundColor = loadColorSetting('backgroundColor', '#FFFFFF');
  const textColor = loadColorSetting('textColor', '#000000');
  ...
  // Обробники для кнопок скидання
  document.getElementById('accentColorResetBtn').addEventListener('click', () => {
    saveColorSetting('accentColor', '#C778DD'); // Скидаємо до стандартного
    document.getElementById('accentColorPicker').value = '#C778DD';
    document.body.style.setProperty('--accent-color', '#C778DD');
  });

  document.getElementById('backgroundColorResetBtn').addEventListener('click', () => {
    saveColorSetting('backgroundColor', '#FFFFFF'); // Скидаємо до стандартного
  ...
  document.getElementById('textColorResetBtn').addEventListener('click', () => {
    saveColorSetting('textColor', '#000000'); // Скидаємо до стандартного
