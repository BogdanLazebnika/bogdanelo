...
function addFadeInOnLoad(id) {
  const placeholder = document.getElementById(id);
  if (!placeholder) return;

  const observer = new MutationObserver((mutationsList, observer) => {
    for (const mutation of mutationsList) {
      if (mutation.type === 'childList' && placeholder.children.length > 0) {
        placeholder.classList.add('visible');
        observer.disconnect(); // відключаємо, бо вже зробили
        break;
      }
    }
  });
...
// Викликаємо для кожного плейсхолдера
document.addEventListener('DOMContentLoaded', () => {
  ['header-placeholder', 'media-sidebar-placeholder', 'options-placeholder', 'footer-placeholder'].forEach(id => {
    addFadeInOnLoad(id);
  });
});
