...
const COLOR_SETTINGS_KEY = 'colorSettings';

// Отримання збережених кольорів
...
// Застосування кольорів до сайту
function applyColorSettings() {
  ...
  if (accentInput) accentInput.value = settings.accent || '#ff6600';
  if (backgroundInput && settings.background) backgroundInput.value = settings.background;
  if (textInput && settings.text) textInput.value = settings.text;
}

// Зміна кольору за ключем
function changeColorSetting(key, value) {
  const settings = getColorSettings();
  settings[key] = value;
...
// Подія після завантаження
document.addEventListener('DOMContentLoaded', () => {
  applyColorSettings();

  // Слухачі на color input
  const accentInput = document.getElementById('accentColorPicker');
  const backgroundInput = document.getElementById('backgroundColorPicker');
  const textInput = document.getElementById('textColorPicker');
  ...
  if (accentInput) {
    accentInput.addEventListener('input', () =>
      changeColorSetting('accent', accentInput.value)
    );
  }

  if (backgroundInput) {
    backgroundInput.addEventListener('input', () =>
      changeColorSetting('background', backgroundInput.value)
    );
  }

  if (textInput) {
    textInput.addEventListener('input', () =>
      changeColorSetting('text', textInput.value)
    );
  }
  ...
  if (accentReset) {
    accentReset.addEventListener('click', () => resetColorSetting('accent'));
  }

  if (backgroundReset) {
    backgroundReset.addEventListener('click', () => resetColorSetting('background'));
  }

  if (textReset) {
    textReset.addEventListener('click', () => resetColorSetting('text'));
  }
});
