...
const defaultSettings = {
  theme: undefined,       // undefined означає стандарт (темна)
  accentColor: '#ff4081', // приклад акценту
  backgroundColor: '#121212', // фон для темної теми
  textColor: '#ffffff'
};

// Застосування налаштувань (теми і кольорів)
function applyUserSettings() {
  ...
  // Акцентний колір -- можна змінити CSS-змінну, наприклад
  if (settings.accentColor) {
...
// Ініціалізація елементів на сторінці (після підвантаження HTML)
function initOptionsControls() {
  const settings = JSON.parse(localStorage.getItem(STORAGE_KEY)) || {};

  // Кнопки теми
  const themeButtons = document.querySelectorAll('button[onclick^="updateUserSetting(\'theme\'"]');
  themeButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      const themeValue = btn.getAttribute('onclick').match(/'theme',\s*(.+)\)/)[1];
      // Перетворимо undefined рядок на JS undefined
      const value = themeValue === 'undefined' ? undefined : themeValue.replace(/['"]/g, '');
  ...
  if (!accentColorPicker || !backgroundColorPicker || !textColorPicker) return;

  // Встановлюємо значення з локалки або дефолтні
  accentColorPicker.value = settings.accentColor || defaultSettings.accentColor;
  backgroundColorPicker.value = settings.backgroundColor || defaultSettings.backgroundColor;
  textColorPicker.value = settings.textColor || defaultSettings.textColor;

  // Обробники зміни кольору
...
// Функція оновлення налаштувань у локалсторедж і застосування
function updateUserSetting(key, value) {
  const settings = JSON.parse(localStorage.getItem(STORAGE_KEY)) || {};
...
// Чекаємо на завантаження компонентів і ініціалізуємо контролери
document.addEventListener('DOMContentLoaded', () => {
  // оскільки у тебе компоненти підвантажуються асинхронно,
  // краще почекати 100-200ms перед ініціалізацією,
  // або викликати initOptionsControls() після завантаження options.html
  setTimeout(() => {
    initOptionsControls();
    applyUserSettings();
  }, 200);
