function updateUserSetting(key, value) {
  ...
  // Логіка додавання/прибрання класу visible на .button-options-container
  const container = document.querySelector('.button-options-container');
  if (container) {
    // Припустимо, ключ 'showOptionsContainer' керує видимістю
    if (key === 'showOptionsContainer') {
      if (value === true) {
...
// І в applyUserSettings(), щоб відновити стан при завантаженні:
function applyUserSettings() {
  const settings = JSON.parse(localStorage.getItem('siteSettings')) || {};
  const container = document.querySelector('.button-options-container');

  if (container) {
    if (settings.showOptionsContainer === true) {
      container.classList.add('visible');
      container.classList.remove('hidden');
    } else {
      container.classList.remove('visible');
