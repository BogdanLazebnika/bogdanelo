...
function updateActiveButtons() {
  ...
  buttons.forEach(button => {
    ...
    // Приводимо збережене у settings[key] до відповідного типу
    let storedValue = settings[key];
    if (storedValue === 'true') storedValue = true;
    else if (storedValue === 'false') storedValue = false;
    else if (storedValue === undefined) storedValue = undefined;

    // Порівняння для кнопки "Стандарт" (undefined)
  ...
  // Акцент
  const accentColor = settings.accentColor || 'var(--accent-color)';
...
function waitForOptionButtonsToLoad(retries = 30) {
  const buttons = document.querySelectorAll('.button-open-options-list button');
  if (buttons.length > 0) {
    updateActiveButtons();
  } else if (retries > 0) {
    setTimeout(() => waitForOptionButtonsToLoad(retries - 1), 100);
  } else {
    console.warn('Option buttons not found in DOM after waiting.');
  }
}

// Запускаємо після window.load
window.addEventListener('load', () => {
  waitForOptionButtonsToLoad();
});
