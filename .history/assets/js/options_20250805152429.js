...
// Оновлення налаштувань і збереження у localStorage
function updateUserSetting(key, value) {
  const settings = JSON.parse(localStorage.getItem('siteSettings')) || {};
  if (value === undefined) {
    delete settings[key];
...
// Застосування налаштувань (тема і кольори)
...
// Оновлення класів кнопок у списку (активна кнопка для теми)
function updateActiveButtons() {
  const settings = JSON.parse(localStorage.getItem('siteSettings')) || {};
  const buttons = document.querySelectorAll('.button-open-options-list-list-two-level button');
  ...
  buttons.forEach(button => {
    ...
    if ((settings[key] === undefined && value === undefined) || settings[key] === value) {
      button.classList.add('active');
    } else {
      button.classList.remove('active');
    }
  ...
  // Скидання акцентного кольору - кнопка active якщо дефолт
  const resetBtn = document.getElementById('accentColorResetBtn');
  if (resetBtn) {
    if (!settings.accentColor) {
      resetBtn.classList.add('active');
...
// Ініціалізація елементів управління кольорами і темою
function initSettingsControls() {
  ...
  if (accentPicker) {
    accentPicker.value = settings.accentColor || '#C778DD';
    accentPicker.addEventListener('input', () => {
      updateUserSetting('accentColor', accentPicker.value);
    });
  }
  if (accentReset) {
    accentReset.addEventListener('click', () => {
      updateUserSetting('accentColor', undefined);
      if (accentPicker) accentPicker.value = '#C778DD';
    });
  }
  ...
  if (backgroundPicker) {
    backgroundPicker.value = settings.backgroundColor || '#0F0F0F';
    backgroundPicker.addEventListener('input', () => {
      updateUserSetting('backgroundColor', backgroundPicker.value);
    });
  }
  if (backgroundReset) {
    backgroundReset.addEventListener('click', () => {
      updateUserSetting('backgroundColor', undefined);
      if (backgroundPicker) backgroundPicker.value = '#0F0F0F';
    });
  }
  ...
  if (textPicker) {
    textPicker.value = settings.textColor || '#FFFFFF';
    textPicker.addEventListener('input', () => {
      updateUserSetting('textColor', textPicker.value);
    });
  }
  if (textReset) {
