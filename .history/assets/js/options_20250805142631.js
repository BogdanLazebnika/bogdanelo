...
function updateUser Settting(key, value) {
  const settings = JSON.parse(localStorage.getItem('siteSettings')) || {};
  ...
  localStorage.setItem('siteSettings', JSON.stringify(settings));
  applyUser Settings(); // Викликаємо функцію для застосування налаштувань
  updateActiveButtons();  // Оновлюємо класи активних кнопок після зміни
}
...
document.addEventListener('DOMContentLoaded', () => {
  loadColorSettings(); // Завантажуємо кольори при завантаженні сторінки

  // Обробники подій для кольорових пікерів
  const accentColorPicker = document.getElementById('accentColorPicker');
  if (accentColorPicker) {
    accentColorPicker.addEventListener('input', () => {
      updateUser Setting('accentColor', accentColorPicker.value);
  ...
  if (backgroundColorPicker) {
    backgroundColorPicker.addEventListener('input', () => {
      updateUser Setting('backgroundColor', backgroundColorPicker.value);
  ...
  if (textColorPicker) {
    textColorPicker.addEventListener('input', () => {
      updateUser Setting('textColor', textColorPicker.value);
  ...
  if (resetBackgroundColorBtn) {
    resetBackgroundColorBtn.addEventListener('click', () => {
      updateUser Setting('backgroundColor', undefined);
      loadColorSettings(); // Оновлюємо кольоровий пікер
    });
  }
  ...
  if (resetTextColorBtn) {
    resetTextColorBtn.addEventListener('click', () => {
      updateUser Setting('textColor', undefined);
      loadColorSettings(); // Оновлюємо кольоровий пікер
    });
  }
});
