...
document.addEventListener("click", function (event) {
  const options = document.getElementById("draggable-options");

  if (!options.classList.contains("visible")) return;
  ...
  const windowResized =
    window.innerWidth !== lastWindowSize.width ||
    window.innerHeight !== lastWindowSize.height;

  // Якщо клік поза вікном і розмір не змінювався — закриваємо
  if (!clickInside && !windowResized) {
