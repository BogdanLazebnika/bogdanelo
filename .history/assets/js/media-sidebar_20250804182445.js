...
document.addEventListener('DOMContentLoaded', () => {
  const sidebar = document.querySelector('.media-sidebar.initial-animate');
  if (sidebar) {
    // Видалимо клас через 800 мс (трохи більше за час анімації)
    setTimeout(() => {
      sidebar.classList.remove('initial-animate');
    }, 800);
  }
});
