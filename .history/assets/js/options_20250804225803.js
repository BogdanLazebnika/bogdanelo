...
let mouseDownInsideWindow = false;

// Слідкуємо за натисканням миші
window.addEventListener('mousedown', (event) => {
  mouseDownInsideWindow = true;
});

// Слідкуємо за відпусканням миші
window.addEventListener('mouseup', (event) => {
  mouseDownInsideWindow = true; // в будь-якому випадку вважаємо, що завершилось у вікні
});
...
document.addEventListener('click', (event) => {
  ...
  // Якщо клік був поза панеллю, не по кнопці, без зміни розміру і мишку не відпустили поза вікном
  if (
    !panel.classList.contains('hidden') &&           // панель відкрита
    !panel.contains(event.target) &&                 // не клік всередині панелі
    !openBtn.contains(event.target) &&               // не клік по кнопці відкриття
    !sizeChanged &&                                  // розмір вікна не змінився
    mouseDownInsideWindow                            // не було drag-out з вікна
  ) {
    closeSettingsPanel();
  }

  // Оновлюємо розміри
  initialWindowWidth = currentWidth;
