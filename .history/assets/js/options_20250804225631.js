...
document.addEventListener('click', (event) => {
  ...
  const clickedOutsideButton = !openBtn.contains(event.target);
  const isPanelVisible = !panel.classList.contains('hidden');

  const windowResized =
    window.innerWidth !== lastWindowWidth || window.innerHeight !== lastWindowHeight;

  if (isPanelVisible && clickedOutsidePanel && clickedOutsideButton && !windowResized) {
    closeSettingsPanel();
  }

  // Після кожного кліка оновлюємо розміри (на випадок справжньої зміни)
