...
function applyColorSettings() {
  ...
  if (settings.textColor) {
    ...
    if (textInput) textInput.value = settings.textColor;
  } else {
    body.style.color = '';
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Тема — ти використовуєш кнопку з updateUserSetting, не чіпаю її

  // Акцент
  const accentInput = document.getElementById('accentColorPicker');
  if (accentInput) {
    accentInput.addEventListener('input', () => {
      saveColorSetting('accent', accentInput.value);
    });
  }
  ...
  if (bgInput) {
    bgInput.addEventListener('input', () => {
      saveColorSetting('backgroundColor', bgInput.value);
    });
  }

  const bgReset = document.getElementById('backgroundColorResetBtn');
  if (bgReset) {
  ...
  if (textInput) {
    textInput.addEventListener('input', () => {
      saveColorSetting('textColor', textInput.value);
    });
  }
