...
const colorInputs = {
    accent: document.getElementById('accentColorPicker'),
    background: document.getElementById('backgroundColorPicker'),
    text: document.getElementById('textColorPicker')
  };

  const resetButtons = {
    accent: document.getElementById('accentColorResetBtn'),
    background: document.getElementById('backgroundColorResetBtn'),
    text: document.getElementById('textColorResetBtn')
  };
  ...
  function applyUserSettings() {
    const settings = getSettings();
    if (settings.accentColor) colorInputs.accent.value = settings.accentColor;
    if (settings.backgroundColor) colorInputs.background.value = settings.backgroundColor;
    if (settings.textColor) colorInputs.text.value = settings.textColor;

    // Застосовуємо до CSS змінних
    document.documentElement.style.setProperty('--accent-color', settings.accentColor || '');
    document.documentElement.style.setProperty('--background-color', settings.backgroundColor || '');
    document.documentElement.style.setProperty('--text-color', settings.textColor || '');
  }

  // Обробники зміни кольорів
  colorInputs.accent.addEventListener('input', e => {
    updateUserSetting('accentColor', e.target.value);
  });

  colorInputs.background.addEventListener('input', e => {
    updateUserSetting('backgroundColor', e.target.value);
  });

  colorInputs.text.addEventListener('input', e => {
    updateUserSetting('textColor', e.target.value);
  });

  // Обробники скидання окремих полів
  resetButtons.accent.addEventListener('click', () => {
    updateUserSetting('accentColor', undefined);
    colorInputs.accent.value = '#000000';
  });

  resetButtons.background.addEventListener('click', () => {
    updateUserSetting('backgroundColor', undefined);
    colorInputs.background.value = '#000000';
  });

  resetButtons.text.addEventListener('click', () => {
    updateUserSetting('textColor', undefined);
    colorInputs.text.value = '#000000';
  });

  // Скидання всіх налаштувань з підтвердженням
  ...
  confirmYes.addEventListener('click', () => {
    ...
    applyUserSettings();
    window.location.reload(); // перезавантажити сторінку для повного очищення
  });
