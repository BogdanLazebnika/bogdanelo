...
window.headerScrollHandler = function () {
  ...
  if (scrollTop > window.lastScrollTop) {
    window.headerPosition = Math.min(window.headerPosition + (scrollTop - window.lastScrollTop), headerHeight);
  } else {
    window.headerPosition = Math.max(window.headerPosition - (window.lastScrollTop - scrollTop), 0);
  }

  // Тимчасово вимикаємо transition
  header.style.transition = 'none';
  header.style.top = Math.max(-headerHeight, -window.headerPosition) + 'px';

  clearTimeout(scrollTimeout);
  scrollTimeout = setTimeout(() => {
    header.style.transition = ''; // повертаємо плавність
  }, 100);
