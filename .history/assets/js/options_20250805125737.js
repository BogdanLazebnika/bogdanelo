...
function initColorPickers() {
  ...
  if (accentPicker) {
    ...
    accentPicker.addEventListener('input', () => {
      setColorSetting('accentColor', accentPicker.value);
    });
  }
  ...
  if (backgroundPicker) {
    ...
    backgroundPicker.addEventListener('input', () => {
      setColorSetting('backgroundColor', backgroundPicker.value);
    });
  }

  if (textPicker) {
    textPicker.value = settings.textColor || '#E0E0E0';
    textPicker.addEventListener('input', () => {
      setColorSetting('textColor', textPicker.value);
    });
  }

  // Reset buttons
  document.getElementById('accentColorResetBtn')?.addEventListener('click', () => {
    resetColorSetting('accentColor');
    accentPicker.value = '#C778DD';
  });

  document.getElementById('backgroundColorResetBtn')?.addEventListener('click', () => {
    resetColorSetting('backgroundColor');
    backgroundPicker.value = '#0F0F0F';
  });

  document.getElementById('textColorResetBtn')?.addEventListener('click', () => {
    resetColorSetting('textColor');
    textPicker.value = '#E0E0E0';
  });
}
