...
function applyUserSettings() {
  ...
  if (header) {
    header.classList.remove('fixed', 'always-fixed'); // скидаємо перед оновленням
    if (settings.fixedHeader === true) {
      header.classList.add('fixed'); // Плаваюча шапка
    } else if (settings.fixedHeader === 'always') {
      header.classList.add('always-fixed'); // Завжди фіксована
    }
    // Стандарт — нічого не додаємо
  }
  ...
  if (settings.accentColor) {
    ...
    html.style.removeProperty('--accent-color'); // або встанови дефолтний
  }

  // === 4. Background Color ===
  if (settings.backgroundColor) {
    html.style.setProperty('--background-color', settings.backgroundColor);
  } else {
  ...
  if (settings.textColor) {
    html.style.setProperty('--text-color', settings.textColor);
  } else {
