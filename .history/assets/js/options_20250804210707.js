...
// Відкрити меню — сховати кнопку, показати меню з анімацією
openBtn.addEventListener('click', () => {
  settingsPanel.classList.add('visible'); // показати меню плавно
  openBtnContainer.classList.add('hidden'); // сховати кнопку
});

// Закрити меню — прибрати клас .visible, потім повернути кнопку
function closeSettingsPanel() {
  settingsPanel.classList.remove('visible'); // запустить анімацію зникнення

  // Зачекай завершення анімації (400ms, як у transition)
  setTimeout(() => {
    openBtnContainer.classList.remove('hidden'); // показати кнопку після зникнення
  }, 400);
}
