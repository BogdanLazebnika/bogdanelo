...
document.addEventListener('click', (event) => {
  ...
  const windowResized =
    currentSize.width !== lastWindowSize.width ||
  ...
  if (
    ...
    // Якщо розмір не змінювався — закриваємо
    if (!windowResized) {
      closeSettingsPanel();
    }

    // Оновлюємо розмір, навіть якщо не resize, бо resize event не завжди тригериться швидко
