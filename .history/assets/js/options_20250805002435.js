...
// 🔁 Ось ця функція гарантує, що всі елементи зʼявились
function waitForAccentColorElements(retries = 10) {
  const picker = document.getElementById('accentColorPicker');
  const resetBtn = document.getElementById('accentColorResetBtn');

  if (!picker || !resetBtn) {
    if (retries > 0) {
      setTimeout(() => waitForAccentColorElements(retries - 1), 100);
    }
    return;
  }
...
// 🔁 Стартуємо коли завантажено скрипт
waitForAccentColorElements();
