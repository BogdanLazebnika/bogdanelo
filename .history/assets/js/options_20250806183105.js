function updateUserSetting(key, value) {
  // 1. Завантажуємо всі поточні налаштування
  const settings = JSON.parse(localStorage.getItem('siteSettings')) || {};

  // 2. Оновлюємо потрібне значення
  if (value === undefined) {
    delete settings[key];
  } else {
    settings[key] = value;
  ...
  // 4. Застосовуємо всі налаштування
  applyUserSettings();

  // 5. Оновлюємо активні кнопки
  updateActiveButtons();

  // 6. Оновлюємо кольори, якщо потрібно
  if (typeof initColorSettings === 'function') {
    initColorSettings();  // Це забезпечує застосування кольорів
  }
}
